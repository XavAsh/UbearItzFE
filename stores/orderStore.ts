"use client";

import { create } from "zustand";
import { createOrder, getOrders } from "@/services/api/orders";
import type { Order, OrderItem, OrderStatus } from "@/types";

type RequestState = "idle" | "loading" | "ready" | "error";

const STORAGE_KEY = "orders-store";

const cleanupLegacyStorage = () => {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.removeItem("orders");
    window.localStorage.removeItem(STORAGE_KEY);
  } catch {
    // ignore
  }
};

/**
 * Order store — always synced from the API (not persisted; avoids stale status after restaurant updates).
 */
type OrderState = {
  orders: Order[];
  status: RequestState;
  error?: string;
  /** @deprecated use refreshOrders */
  hydrateFromMock: (force?: boolean) => Promise<void>;
  refreshOrders: () => Promise<void>;
  placeOrder: (input: { restaurantId: string; items: OrderItem[] }) => Promise<Order>;
  clear: () => void;
  getOrderById: (id: string) => Order | undefined;
  getOrdersByStatus: (status: OrderStatus) => Order[];
  hasOrders: () => boolean;
  latestOrder: () => Order | undefined;
};

export const useOrderStore = create<OrderState>()((set, get) => {
  cleanupLegacyStorage();

  return {
    orders: [],
    status: "idle",
    error: undefined,
    async refreshOrders() {
      if (get().status === "loading") return;
      set({ status: "loading", error: undefined });
      try {
        const orders = await getOrders();
        set({ orders, status: "ready" });
      } catch (err) {
        const message = err instanceof Error ? err.message : "Unable to fetch orders";
        set({ status: "error", error: message });
      }
    },
    async hydrateFromMock(force = true) {
      if (!force && get().orders.length > 0) {
        set({ status: "ready" });
        return;
      }
      return get().refreshOrders();
    },
    async placeOrder({ restaurantId, items }) {
      set({ status: "loading", error: undefined });
      try {
        const order = await createOrder({ restaurantId, items });
        set({ orders: [order, ...get().orders], status: "ready" });
        return order;
      } catch (err) {
        const message = err instanceof Error ? err.message : "Unable to place order";
        set({ status: "error", error: message });
        throw err;
      }
    },
    clear() {
      set({ orders: [], status: "idle", error: undefined });
    },
    getOrderById(id) {
      return get().orders.find((order) => order.id === id);
    },
    getOrdersByStatus(status) {
      return get().orders.filter((order) => order.status === status);
    },
    hasOrders() {
      return get().orders.length > 0;
    },
    latestOrder() {
      return get().orders[0];
    },
  };
});

